...
class SearchService {
  static async list(params: IHomeSearchRequestParams): Promise<IHomeResult[]> {
    ...
    //  try {
    //      const response: AxiosResponse<IHomeResult[]> = await axios.get(`${INDEX}`, { params });
    //      return response.data;
    //  } catch (error) {
    //      SearchService.handleError(error as AxiosError);
